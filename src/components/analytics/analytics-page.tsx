...
function BarChart({ data, className = "h-60 md:h-80" }: { data: number[]; className?: string }) {
  if (data.length === 0)
    return (
...
export function AnalyticsPage() {
  ...
  return (
    <DashboardPageShell width="wide" className="space-y-5">
      <PageHeader
        title="Analytics"
        description="Risk trends, segment breakdown, and cohort movement."
        actions={
          <Toolbar mobileScroll className="shrink-0">
            {PERIODS.map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
