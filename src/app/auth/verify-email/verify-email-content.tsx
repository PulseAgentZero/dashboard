...
export default function VerifyEmailContent() {
  ...
  const { org, user, refetch } = useAuth();
  ...
  function handleContinue() {
    postAuthRedirect(org, router, user);
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
          {state === "loading" && (
            <>
              ...
              <p className="mt-4 text-[13px] text-slate-600">Verifying…</p>
            </>
          )}
          {state === "success" && (
            <>
              ...
              <button
                ...
                className="mt-6 w-full rounded-xl bg-blue-600 py-3 text-[13px] font-semibold text-white hover:bg-blue-700"
              >
                Continue
              </button>
            </>
          ...
          {state === "error" && (
            <>
              ...
              <Link
                href="/auth/login"
                className="mt-6 inline-block text-[13px] font-medium text-blue-600 hover:text-blue-500"
              >
                Back to sign in
              </Link>
            </>
    ...
    >
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
        ...
        <button
          ...
          onClick={() => resend()}
          disabled={resending}
          className="mt-2 w-full rounded-xl border border-slate-200 bg-white py-2.5 text-[13px] font-medium text-slate-700 hover:bg-slate-50 transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
